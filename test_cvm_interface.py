...
def test_cvm_configuration():
    ...
    # Test individual module creation (mock)
    print_subsection("Module Interface Testing")
    
    # Create a mock module for interface testing
    class MockBus:
        pass
    
    mock_bus = MockBus()
    test_module = CVM24PModule(mock_bus, 0x10, "TEST_MODULE_001", 0)
    
    print(f"✅ Module created:")
    print(f"   • Serial: {test_module.serial}")
    print(f"   • Address: 0x{test_module.address:X}")
    print(f"   • Module ID: {test_module.module_id}")
    print(f"   • Initialized: {test_module.is_initialized}")
    print(f"   • Channel count: {len(test_module.channel_voltages)}")
...
def main():
    ...
    try:
        ...
        print(f"   → XC2 libraries: {'Available' if 'XC2_AVAILABLE' in globals() else 'Need installation'}")
